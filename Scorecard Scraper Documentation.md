...
The `UFC Scorecard Scraper` collects official fight scorecards from the UFC website, downloading scorecard images and organizing them by event. It compiles these images into PDFs for each event, allowing the images to be easily reviewed or parsed with an OCR engine.
